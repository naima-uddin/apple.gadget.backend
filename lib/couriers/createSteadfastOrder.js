...
function steadfastHttpError(text, status, data = null) {
  ...
  if (isSteadfastAccountInactive(combined)) {
    return (
      "Steadfast merchant account is not active. The API Key/Secret are fine — " +
      "the test connection balance check may pass, but to book a parcel the merchant account must be activated/approved by Steadfast. " +
      "Log in to the steadfast.com.bd merchant panel to check the account status, or contact Steadfast support (09678-045045)."
    );
  }

  if (status === 401) {
    return `Steadfast authentication failed (HTTP 401). Please verify the API Key/Secret Key.${snippet ? ` (${snippet})` : ""}`;
  }
...
export async function testSteadfastConnection(creds, storeConfig = {}) {
  ...
  return {
    ok: true,
    message:
      "Steadfast API connected (balance check OK). To book a parcel, the merchant account must be active/approved by Steadfast.",
