...
async function getStoreName() {
  ...
  try {
    ...
    _storeNameCache = s?.storeName || process.env.STORE_NAME || "AppleBD";
  } catch {
    _storeNameCache = process.env.STORE_NAME || "AppleBD";
  }
...
export async function sendOrderConfirmationEmail(order) {
  ...
  try {
    await transporter.sendMail({
      ...
      to: toEmail,
      bcc: ORDER_EMAIL,
      subject: `Order Confirmed — ${order.orderNumber || shortId(order._id)} | ${STORE_NAME}`,
...
export async function sendAdminOrderNotification(order) {
  ...
  try {
    await transporter.sendMail({
      ...
      to: ADMIN_EMAIL,
      subject: `New Order ${order.orderNumber || shortId(order._id)} — ৳${(order.total || 0).toFixed(2)} | ${order.paymentMethod}`,
...
<table width="100%" cellpadding="0" cellspacing="0">
  ...
  // Notify both the support mailbox and the store admin (deduplicated so the
  // same inbox isn't emailed twice when they happen to match).
  const adminRecipients = [
    ...new Set([SUPPORT_EMAIL, ADMIN_EMAIL].filter(Boolean)),
  ];
  ...
  const recipients = [adminRecipients.join(", "), email];
  results.forEach((r, i) => {
