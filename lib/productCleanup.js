// Shared permanent-delete logic for products, used by both the admin
// "delete permanently" action and the trash-cleanup cron job. Removes the
// product's image assets (local disk or legacy Cloudinary), detaches any linked
// barcodes, then deletes the document. Kept self-contained so it has no
// dependency on the admin router.
import { destroyAsset } from "./assetStore.js";
import Product from "../models/Product.js";
import Barcode from "../models/Barcode.js";
import { clearProductsCache, clearProductCache } from "./redis.js";

// Retention window before a trashed product is permanently removed (30 days).
export const TRASH_RETENTION_MS = 30 * 24 * 60 * 60 * 1000;

// Permanently delete every product matched by `filter`. Returns the count of
// documents removed.
export async function permanentlyDeleteProducts(filter) {
  const products = await Product.find(filter);
  let deleted = 0;
  for (const p of products) {
    if (Array.isArray(p.images) && p.images.length > 0) {
      for (const img of p.images) {
        if (img && img.public_id) {
          // A duplicated product (created before images were copied) may still
          // share this file. Only destroy it when no OTHER product references
          // the same public_id — otherwise the sibling's image would 404.
          const stillUsed = await Product.exists({
            _id: { $ne: p._id },
            "images.public_id": img.public_id,
          });
          if (stillUsed) continue;
          // destroyAsset never throws — best-effort cleanup, never blocks delete
          await destroyAsset(img.public_id, { resource_type: "image" });
        }
      }
    }

    // detach any barcodes still pointing at this product
    await Barcode.updateMany(
      { product: p._id },
      { $set: { product: null, productTitle: "" } },
    );
    await Product.deleteOne({ _id: p._id });
    clearProductCache(p._id);
    deleted++;
  }
  if (deleted > 0) clearProductsCache();
  return deleted;
}
